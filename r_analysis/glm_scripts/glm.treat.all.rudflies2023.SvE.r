...
for(g in 1:nrow(haf.freq.filt.S.E)) { #nrow(haf.freq.filt.S.E)
  tryCatch({
    ...
    #sapply(glmdf, class)
    glmres.S.E[[g]] <- glm(count~ treat.fix, data = glmdf, family = quasipoisson, na.action = na.exclude)
    #summary(emtemp)$p.value
    contrastout.treat.S.E <- rbind(contrastout.treat.S.E,as.data.frame(emmeans(glmres.S.E[[g]], pairwise ~ treat.fix)$contrasts)$p.value)
  }, error=function(e){})
}

#save results so we don't have to do this again
write.table(contrastout.treat.S.E, file="rudflies_2023_SvE_treat.all.GLMcontrast.txt", sep="\t", quote = FALSE, row.names = F)
...
write.table(contrastout.treat.S.E.table, file="rudflies_2023_SvE_treat.all.table.GLMcontrast.txt", sep="\t", quote = FALSE, row.names = F)
