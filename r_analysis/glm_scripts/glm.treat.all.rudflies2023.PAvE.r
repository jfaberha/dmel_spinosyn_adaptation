...
#filter for founder samples for E and PA
...
for(g in 1:nrow(haf.freq.filt.PA.E)) { #nrow(haf.freq.filt.PA.E)
  tryCatch({
    ...
    #sapply(glmdf, class)
    glmres.PA.E[[g]] <- glm(count~ treat.fix, data = glmdf, family = quasipoisson, na.action = na.exclude)
    #summary(emtemp)$p.value
    contrastout.treat.PA.E <- rbind(contrastout.treat.PA.E,as.data.frame(emmeans(glmres.PA.E[[g]], pairwise ~ treat.fix)$contrasts)$p.value)
  }, error=function(e){})
}

#save results so we don't have to do this again
write.table(contrastout.treat.PA.E, file="rudflies_2023_PAvE_treat.all.GLMcontrast.txt", sep="\t", quote = FALSE, row.names = F)
...
write.table(contrastout.treat.PA.E.table, file="rudflies_2023_PAvE_treat.all.table.GLMcontrast.txt", sep="\t", quote = FALSE, row.names = F)
